...
public class Serverj {
	...
	public static void main(String[] args) {
		...
		try {
			...
			DataInputStream serverInputStream=new DataInputStream(socket.getInputStream());  //TCP接收
			DataOutputStream serverDataOutputStream=new DataOutputStream(socket.getOutputStream());  //TCP发送
			int i=0;
			while (i<4) {
			Arrays.fill(inbuf, (byte) 0);
			socket2.receive(recievePacket);  //UDP接收
			bsleep = Double.parseDouble(new String(inbuf).trim()); 
			asleep=serverInputStream.readDouble(); //TCP接收
			as=serverInputStream.readInt(); //TCP接收
			Arrays.fill(inbuf, (byte) 0);
			socket2.receive(recievePacket); //UDP接收
			...
			i++;
			serverDataOutputStream.writeBoolean(true); //TCP发送，比较完成
			}
